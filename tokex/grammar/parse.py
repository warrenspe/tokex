...
def tokenize_grammar(grammar_string):
    ...
    all_flags_re = re.compile(flags_re_string(flags.__all__))
...
def construct_grammar(grammar_string, allow_sub_grammar_definitions=False):
    ...
    for token_idx, token_dict in enumerate(grammar_tokens):
        ...
        token = token_dict["element_token"]

        # Openers
        if token == "{":
            element = elements.OneOfSet(token, flags)
            grammar_stack[-1].add_sub_element(element)
            grammar_stack.append(element)

        elif token[:2] == "*(":
            element = elements.ZeroOrMore(token, flags)
            grammar_stack[-1].add_sub_element(element)
            grammar_stack.append(element)

        elif token[:2] == "+(":
            element = elements.OneOrMore(token, flags)
            grammar_stack[-1].add_sub_element(element)
            grammar_stack.append(element)

        elif token[:2] == "?(":
            element = elements.ZeroOrOne(token, flags)
            grammar_stack[-1].add_sub_element(element)
            grammar_stack.append(element)

        elif token[0] == "(":
            element = elements.Grammar(token, flags)
            grammar_stack[-1].add_sub_element(element)
            grammar_stack.append(element)

        elif token[0] == "<":
            element = elements.NamedElementToken(token, flags)
            grammar_stack[-1].add_sub_element(element)
            grammar_stack.append(element)

        elif token[:3].lower() == "sep":
            element = elements.IteratorDelimiter(token, flags)
        ...
        # Closers
            ...
            if isinstance(grammar_stack[-1], (elements.ZeroOrMore, elements.ZeroOrOne, elements.OneOrMore, elements.NamedGrammar)):
                grammar_stack.pop()
            ...
            if isinstance(grammar_stack[-1], (elements.NamedElementToken)):
                grammar_stack.pop()
        ...
        # Singular tokens
            ...
            grammar_stack[-1].add_sub_element(elements.StringLiteral(token, flags))

        elif token[0] == "~":
            grammar_stack[-1].add_sub_element(elements.RegexString(token, flags))

        elif token[0] in ("'", '"'):
            grammar_stack[-1].add_sub_element(elements.StringLiteral(token, flags))

        # Sub Grammar open
        if token[:3].lower() == "def":
            ...
            element = elements.SubGrammarDefinition(token, flags)
            grammar_stack.append(element)
        ...
        # Sub Grammar Usage
            ...
            sub_grammar_name = elements.SubGrammarUsage(token, flags).name
