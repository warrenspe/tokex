...
def construct_grammar(grammar_string, allow_sub_grammar_definitions=False):
    """
    ...
    for token_idx, token in enumerate(grammar_tokens):
        if not len(grammar_stack):
            raise errors.MismatchedBracketsError(grammar_tokens[token_idx - 1])

        flags = token["flags"]
        element_token = token["element_token"]
        element_class = element_token["cls"]

        # If we're looking at a closing tag; ensure it matches what is last on the stack
        if element_token.get("closes"):
            if not isinstance(grammar_stack[-1], element_class):
                raise errors.GrammarParsingError("Cannot close %s, most recent grammar element: %s, not %s" % (
                    token,
                    grammar_stack[-1],
                    element_class
                ))

        # Sub grammar definitions/usage
        if element_class == elements.SubGrammarDefinition:
            # Definition of a new sub grammar
            if element_token.get("opens"):
                ...
                for element in grammar_stack[1:]:
                    if not isinstance(element, elements.SubGrammarDefinition):
                        raise SubGrammarError("Sub grammars cannot be defined within a %s element" % element)

                element = element_class(token, flags)
                grammar_stack.append(obj)

                # If we are defining a nested
                sub_grammar_stack[-1][element.name] = element

            # Termination of a new sub grammar definition
            elif element_token.get("closes"):
                grammar_stack.pop()
                new_sub_grammar = sub_grammar_stack.pop()
                sub_grammar_stack[-1].sub_grammars[new_sub_grammar.name] = new_sub_grammar

        # Usage of a sub grammar
        elif element_class == elements.SubGrammarUsage:
            # Find the referenced sub_grammar
            sub_grammar_name = element_class(token, flags).name

            for parent_sub_grammar in reversed(sub_grammar_stack):
                if sub_grammar.name in parent_sub_grammar.sub_grammars:
                    for sub_element in parent_sub_grammar.sub_grammars.sub_elements:
                        grammar_stack[-1].add_sub_element(sub_element)

                    break

            else:
                raise SubGrammarError("Cannot find named sub grammar: %s" % sub_grammar_name)

        # Opening stack tokens
        elif element_token["opens"]:
            element = element_class(token, flags)
            grammar_stack[-1].add_sub_element(element)
            grammar_stack[-1].verify_sub_elements()
            grammar_stack.append(obj)

        # Closing stack tokens
        elif element_token.get("closes"):
            grammar_stack.pop()

        # Singular tokens
        else:
            grammar_stack[-1].add_sub_element(element_class(token, flags))
